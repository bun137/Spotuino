...
import pyautogui, time, serial, subprocess, os
...
arduino_uno = serial.Serial('/dev/ttyUSB0', 9600)
...
def get_song(old_song):
    ...
    current_song = subprocess.check_output(" playerctl metadata | grep title | awk -F'               ' '{print $2}' ", shell=True, universal_newlines=True)
    ...
    if current_song != old_song:
        ...
        arduino_uno.write(current_song.encode())
